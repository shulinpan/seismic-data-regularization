...
"Data reconstruction of actual post_stk data with trained conv network model"
...
mask_generator = MaskGenerator(segydata.shape[0],segydata.shape[1],rand_seed=220)#220
mask = mask_generator.sample()

masked_data = deepcopy(segydata)
masked_data[mask==0] = 0

model = ResNet( inference_only=True)
model.load(os.path.join(os.getcwd(),'conv_model.h5'))
...
pred_datas = model.predict(chunked_datas)
